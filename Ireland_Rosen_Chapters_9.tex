...
{\bf Lemma.} Let $a \in \Z$, $a \equiv -1 \pmod 3$, and $b\in \Z$ such that $a \wedge b = 1$. Then $\chi_a(b)= 1$.
...
We don't suppose in this proof that $\pi$ is a prime element, so $p = N(\pi)$ is not necessarily prime. Here ``primary element " means that $\pi \equiv -1 \pmod 3$, and that $\pi$ is not a unit.
\begin{enumerate}
...
\item[(c)]  
Suppose first that $a$ is not a unit, which permits to consider $\chi_a$. As $\pi,a$ are primary, by Exercise 9.20, $\chi_\pi(a) = \chi_a(\pi)$.
...
Here $a$ is relatively prime to $b$ in $\Z$ : if a rational prime $r$ divides $a,b$, then $r \mid \pi$ in $D$, thus $r \mid \overline{\pi}$, so $r^2 \mid \pi \overline{\pi} = p$ in $D$, thus $r^2 \mid p$ in $\Z$, which is absurd. The Lemma gives then $\chi_a(b) = 1$.
 ...
 If $a$ is a unit, since $a \in \Z, a \equiv -1 \pmod 3$, then $a = -1$ and $m=0$, so $$\chi_\pi(a) = 1 = \omega^m.$$
 
 In all cases, $\chi_\pi(a) = \omega^m$.
 \item[(d)] 
...
\begin{proof}
We suppose in this exercise that $a+b$ is not a unit. The other case will be examined in the conclusion (Exercise 26).
\begin{enumerate}
...
\begin{proof}
If $a +b $ is not a unit, then $\pi$ and $a+b$ are primary elements of $D$, so
...
If $a+b$ is a unit, since $a+b \equiv -1 \pmod 3$, then $a+b = -1$, so $3m-1 + 3n = -1$, therefore $m = -n$.

By Exercise 9.25, 
$$1 = \chi_\pi(-1) = \chi_\pi(a+b) = \omega^{2n} \chi_\pi(\lambda),$$ therefore
$$\chi_\pi(\lambda) = \omega^{-2n} = \omega^{2m}.$$
In each case, $\chi_\pi(\lambda) = \omega^{2m}$.
\end{proof}
