...
Therefore $a^{2n} \equiv 1 \pmod p$. As $a$ is a primitive root modulo $p$, $p-1 \mid 2n$, $2t \mid n$, so $n$ is even.

Hence $a^n \equiv 1 \pmod p$, and  $p-1 \mid n$. So the least $n\in \N^*$ such that $(-a)^n \equiv 1 \pmod p$ is $p-1$ : the order of $-a$ modulo $p$ is $p-1$, $-a$ is a primitive root modulo $p$.
...
Conversely, suppose that $-a$ has order $(p-1)/2 = 2t+1$ modulo $p$. Let $2,p_1,\ldots p_k$ the prime factors of $p-1$, where $p_i$ are odd.
...
As above, if we suppose that $3$ is not a primitive root modulo $p$, then $3^{2^{n-1}}  \equiv 1 \pmod p$, so  $n\geq 2$, and $(-3)^{(p-1)/2} = 3^{2^{n-1}}  \equiv 1 \pmod p$, so $-3$ is a square modulo $p$ : there exists $a \in \Z$ such that $-3 \equiv a^2 \pmod p$. 
...
Conversely, if $u\wedge k = 1$, there exist $u,v \in \Z$ such that $un+vk=1$, so $g = g^{un+vk} = (g^n)^u(g^k)v = x^v \in \langle x \rangle$, so $G \subset \langle x \rangle$, $G = \langle x \rangle$ : $x$ is a generator of $G$.
...
$\bullet$ If $(ab)^k = e$, then $a^k= b^{-k}$, so $a^{kn} = b^{-kn} = (b^n)^{-k} = e$, so $m\mid kn$, with $m\wedge n = 1$, so $m \mid k$.
...
Let $\psi(d)$ the number of elements in $G$ of order $d$ ( $\psi(d) = 0$ if $d \nmid n$). Then $\sum_{c\mid d} \psi(c) = d$. Applying the Mšbius inversion theorem, $\psi(d) = \sum_{c\mid d} \mu(c) d/c=\phi(d)$ (Prop. 2.2.5), in particular, $\psi(n) = \phi(n)\geq 1$. This proves the existence of an element of order $n$ in $G$, so $G$ is cyclic.
...
\end{align*}
The same is true for $N'$, so it is suffisiant to prove that
...
$\bullet$ Suppose that $p\mid a$. The solutions of $\overline{u} \overline{v} = \overline{0}$ are $(\overline{0},\overline{0})$, $(\overline{0},\overline{v})$ for each $\overline{v} \ne \overline{0}$, $(\overline{u},\overline{0})$ for each $\overline{v} =\overline{0}$, that is to say $N = 1+(p-1)+(p-1) = 2p-1$ solutions.
...
Thus $\legendre{-3}{p} = 1$.
\end{proof}
The converse is also true for an odd prime $p$ :  if $\legendre{-3}{p} = 1$, then there exists $a\in \F_p^*$ such that $-\overline{3} =a^2$. $\rho = \frac{-1+a}{2}$ has order  $3$. Indeed
...
\end{align*}
$\prod_{k=1}^{p-1} \zeta^{-k} =\left( \zeta^p\right)^{-(p-1)/2} = 1$. Moreover, $\prod_{k=1}^{p-1} (1 - \zeta^{3k}) = \prod_{k=1}^{p-1} (1 - \zeta^{k})$, since $\overline{k} \mapsto 3 \overline{k}$ is a bijection in $\F_p^*$, so $P = 1$, and consequently
\begin{align*}
...
& \iff \frac{p}{2} < 3j < p
\end{align*}
Let $\mu$ the number of integers $j, 1 \leq j \leq (p-1)/2$ such that their least remainder is negative. Since $3 \leq 3j \leq 3 (p-1)/2$ and $3j \ne p/2$, these $j$ are such that  $\frac{p}{2} < 3j < p$, so $\mu = \nu$.
Therefore
...
The polynomial $f_1(x)f_2(x) = \sum_{k=0}^r c_k x^k$ is primitive (Ex. 6.4). As $vh(x)(x) = u f_1(x)f_2(x)$, $v \mid uc_k$ , and $u\wedge v = 1$, thus $v \mid c_k, k=0,1,\ldots,r$. As $c_1 \wedge \cdots c_k = 1$, $v \mid 1$, so $v = \pm 1$. $h(x) = u f_1(x) f_2(x)$ is monic, thus $u=1$, and $f_1,f_2$ are monic. From $f(x) = \frac{c}{d} f_1(x)$ we deduce $\frac{c}{d} = 1$ and $f(x) = f_1(x) \in \Z[x]$.
...
The Euclidean division gives $P = Q_1 (x^2+mx+n) + R, \ Q_1,R  \in \Q[x], \deg(R) < 2$, so $R = rx+s,\ r,s \in \Q$. So $z = Q_1(\alpha) (\alpha^2+m\alpha+n) + r\alpha + s = r\alpha + s$ :
...
Note : $\alpha = 2 \omega + 1 = \omega - \omega^2 = g$, the quadratic Gauss sum for $p=3$.
\end{proof}
...
\begin{proof}(solution given by agrounds)
...
Congruence holds because $gj$ also runs over a complete system of nonzero residues mod $p$. If $g^n \not \equiv 1$, then $S \equiv 0$. If $g^n \equiv 1$, then $j^n \equiv 1$ for all $j$, hence $S \equiv p-1 \equiv -1$.
