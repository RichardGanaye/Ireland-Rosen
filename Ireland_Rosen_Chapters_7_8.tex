...
If the characteristic of $F$ is 2, $q = 2^k$ for some integer $k\geq 1$, and for all $x \in F$, $x = x^q = \left (x^{2^{k-1}}\right)^2$. So all elements in $F$ or $K$ are squares.
...
$$(H\cdot f)(x) =(x-F\alpha)(x-F^2\alpha)(x- F^3\alpha)\cdots (x-F^{n}\alpha).$$
...
Consequently, the coefficient $\binom{q}{1} = q$ of $x^{p^a}$ is null, so $p \mid q$ : this is absurd. Therefore $q=1$ and $n = p^a$.
\end{proof}
...
Moreover, if $s = 0, t= 1$, then $\chi(t) = 1$, and if $s=1,t=0$, then $\rho(s) = 1$, so
...
$a \equiv 1 \pmod {2+2i}$, and by complex conjugation, $a\equiv 1 \pmod{2-2i}$, so $(2+2i)(2-2i) = 8 \mid (a-1)^2$, thus $4 \mid a-1$.
