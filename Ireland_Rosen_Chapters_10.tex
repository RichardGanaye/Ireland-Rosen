...
$\bullet$ Suppose first that every point $[u_0,u_1,u_2]$ of $\mathscr {C} \cap l$ is such that $u_1 \ne 0$.
...
Therefore, if $|\mathscr {C} \cap l| >n$, then $P(x) =  f\left(\alpha + \beta x,1,x\right) = 0$.


\bigskip
$\bullet$ Now suppose that some point $[u_0,u_1,u_2] \in \mathscr {C} \cap l$ satisfies $u_1 = 0$. Then $[u_0,u_1,u_2] = [\beta u_2, 0, u_2] = [\beta, 0, 1]$. There is exactly one point on $\mathscr {C} \cap l$ with $u_1 = 0$.

By the previous computation, the other points $[u_0,u_1,u_2] \in \mathscr {C} \cap l$, such that $u_1 \ne 0$, satisfy $$0 =  f\left(\alpha + \beta \frac{u_2}{u_1},1,\frac{u_2}{u_1}\right) = P\left(\frac{u_2}{u_1}\right),$$
...
$$f(x,y,z) = \sum_{(i,j) \in \gcro 0, n\dcro^2, i + j \leq n} a_{i,j}\,  x^iy^j z^{n-i-j}.$$
...
&=  \sum_{(i,j) \in \gcro 0, n\dcro^2, i + j \leq n} a_{i,j}\, (\alpha + \beta x)^i x^{n-i-j}\\
&= \sum_{(i,k) \in \gcro 0, n\dcro^2} a_{i, k-i} (\alpha + \beta x)^i x^{n-k}\qquad \qquad (k = i+j).
\end{align*}
If $j>0$, then $\deg((\alpha + \beta x)^i x^{n-i-j}) < n$. Therefore the coefficient of $x^n$ in $P(x)$ is $\sum\limits_{i=0}^n a_{i,0} \beta^i = 0$, thus $\deg(P) \leq n-1$.
If $P\ne 0$, then $P$ has at most $n-1$ roots, and so $\mathscr{C}$ has at most $n-1$ points $[u_0,u_2,u_2]$ such that $u_1 \ne 0$ on $l$. With the unique point $[\beta, 0 ,1]$ such that $u_1= 0$, we obtain at most $n$ points in $ \mathscr {C} \cap l$.
...
\begin{align*}
Q(x) &= x^n  \sum_{(i,k) \in \gcro 0, n\dcro^2}a_{i, k-i} \left(\alpha + \beta \frac{1}{x}\right)^i \frac{1}{x^{n-k}}\\
&=\sum_{(i,k) \in \gcro 0, n\dcro^2}a_{i, k-i} \left(\alpha x+ \beta \right)^i x^{k-i}.
\end{align*}
If $P = 0$ ,then $Q = 0$, and
...
{\bf Second proof} (with same beginning.)
...
Consider the polynomial $g(x,y) = f(\alpha x + \beta y,x, y)$. Then $g$ is homogeneous of degree $n$. A point $[u_0,u_1,u_2]$ of $l$ is on $\mathscr{C}$ if and only if  $g(u_1,u_2) = 0$.
...
If $ u_2^{(1)} \ne 0$, then $g\left(\frac{u_1^{(1)}}{ u_2^{(1)}},1\right) = 0$. Therefore $\frac{u_1^{(1)}}{ u_2^{(1)}}$ is a root of the polynomial  $g(t,1) \in F[t]$, thus $g(t,1) = \left(t - \frac{u_1^{(1)}}{ u_2^{(1)}}\right) h(t)$, where $h(t) \in F[t]$.
...
Since $u_2^{(1)}u_1^{(2)} - u_1^{(1)} u_2^{(2)} \ne 0$, then $h_1(u_1^{(2)}, u_2^{(2)}) = 0$, and we can continue with $h_1$ in place of $g$. By induction
$$g(x,y) = (u_2^{(1)}x - u_1^{(1)}y)(u_2^{(2)}x - u_1^{(2)}y)\cdots (u_2^{(m)}x - u_1^{(m)}y) h_m(x,y),\qquad h_m(x,y) \in F[x,y].$$

This equality shows that either $g(x,y) = 0$, or $n = \deg(g) \geq m$. This proves that if $m = |\mathscr C \cap l |>n$, then $g(x,y) = 0$. In this case, every point $[u_0,u_1,u_2] \in l$ satisfies
$f(\alpha u_1 + \beta u_2, u_1,u_2) = g(u_1,u_2)=0$, thus $l \subset \mathscr{C}$.
...
If $\chi \ne \varepsilon$, there is an $a \in \F_q^*$ such that $\chi(a) \ne 1$. Then, if $T = \sum_{t\in \F_q} \chi(t)$, then
